                                  ...
                                  const std::vector< std::vector<cv::Point2f> >& imagePoints)
{
    ...
    Parameters params = getParameters();//初始化相机的参数,在没处理前参数都是默认值
    // 畸变系数
    params.k1() = 0.0;
    params.k2() = 0.0;
    params.p1() = 0.0;
    ...
    size_t nImages = imagePoints.size();//符合要求的样本个数
    ...
    setParameters(params);//将求出的相机内参参数更新道相机对象中
}
                                       ...
                                       cv::Mat& rvec, cv::Mat& tvec) const
{
    ...
    // objectPoints表示特征点(角点)相对于世界坐标系(棋盘格左上角点为坐标原点)的坐标(三维,Z=0)
    // Ms表示特征点相对于像素坐标系的坐标(二维)
                                  ...
                                  std::vector<cv::Point2f>& imagePoints) const
{
    ...
    for (size_t i = 0; i < objectPoints.size(); ++i)
    {
        ...
        P = R * P + t;//世界坐标系中的3d点坐标转化为相机坐标系中的3D坐标
